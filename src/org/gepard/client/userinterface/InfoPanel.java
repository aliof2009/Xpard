...
import org.gepard.UTFANSIMapper;
...
//		boolean reverse=true;
        ...
        for (int i = -backward; i <= forward; i++) {
            ...
            if (a != -1) {
                strseq1[j] = submat.reverseMap(a);
            } else {
                strseq1[j] = ' ';
            }

            if (b != -1) {
                strseq2[j] = submat.reverseMap(b);
            } else {
                strseq2[j] = ' ';
            }
        ...
        if (seq1pos - backward - offset1 >= 0) {
            insertNumIntoCharArray(strpos1, 0, seq1pos + forward - 1/*seq1pos-backward*/);
        }
        insertNumIntoCharArray(strpos1, forward, seq1pos);
        if (seq1pos + forward - offset1 <= seq1.length) {
            insertNumIntoCharArray(strpos1, oddadd + forward + backward - numLength(seq1pos + forward - 1), seq1pos - backward/*seq1pos+forward-1*/);
        }
        // generate second position string
        if (!complementary) {
            if (seq2pos - backward - offset2 >= 0) {
                insertNumIntoCharArray(strpos2, 0, seq2pos + forward - 1/*seq2pos-backward*/);
            }
            insertNumIntoCharArray(strpos2, forward, seq2pos);
            if (seq2pos + forward - offset2 <= seq2.length) {
                insertNumIntoCharArray(strpos2, oddadd + forward + backward - numLength(seq2pos + forward - 1), seq2pos - backward/*seq2pos+forward-1*/);
            }
        } else {
    ...
    private void constructMarker(int alignlength) {
        ...
        cmarkers[0] = '|';
        for (i = 1; i < alignlength / 2; i++) {
            cmarkers[i] = ' ';
        }
        cmarkers[i] = '|';
        for (i = alignlength / 2 + 1; i < alignlength - 1; i++) {
            cmarkers[i] = ' ';
        }
        cmarkers[i] = '|';
